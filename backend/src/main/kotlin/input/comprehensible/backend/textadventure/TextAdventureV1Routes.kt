...
fun Route.textAdventureV1Routes(textAdventureService: TextAdventureGenerationService) {
    authenticate("account-bearer") {
        ...
        post("/v1/adventures/{adventureId}/messages") {
            val principal = call.principal<AccountSessionPrincipal>() ?: return@post call.respond(HttpStatusCode.Unauthorized)
            val adventureId = requireNotNull(call.parameters["adventureId"])
            val request = call.receive<RespondTextAdventureV1Request>()
            if (request.playerText.isBlank()) {
                return@post call.respond(HttpStatusCode.BadRequest)
            }
            val response = textAdventureService.respondToAdventureForAccount(
                accountId = principal.accountId,
                adventureId = adventureId,
            ...
            when (response) {
                ...
                RespondToAdventureForAccountResult.AdventureEnded -> call.respond(HttpStatusCode.Conflict)
                is RespondToAdventureForAccountResult.Success -> call.respond(HttpStatusCode.Created, response.response)
            }
        }
...
@Serializable
private data class RespondTextAdventureV1Request(
