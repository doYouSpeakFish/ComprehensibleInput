...
abstract class CompareKoverCoverageSnapshotsTask : DefaultTask() {
    @get:InputDirectory
    ...
    fun verifySnapshots() {
        ...
        if (!Files.exists(snapshotRoot)) {
            throw GradleException("Snapshot directory does not exist at $snapshotRoot. Run koverCoverageSnapshot.")
        }
        ...
        val contentDifferences = generatedFiles.keys
            ...
            .filter { relativePath ->
                !normalizeLineBreaks(Files.readString(generatedFiles.getValue(relativePath)))
                    .contentEquals(
                        normalizeLineBreaks(Files.readString(snapshotFiles.getValue(relativePath)))
                    )
            }

        if (missingInSnapshots.isNotEmpty() || missingInGenerated.isNotEmpty() || contentDifferences.isNotEmpty()) {
            val message = buildString {
                appendLine("Kover coverage snapshots are out of date.")
                ...
                appendLine("Run koverCoverageSnapshot to update snapshots.")
            }
