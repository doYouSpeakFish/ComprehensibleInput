...
fun Route.accountRoutes(accountService: AccountService) {
    post("/v1/users") {
        ...
        call.respond(accountService.createAccount(request.email, request.password, call.emailLanguage()).status)
    }
    ...
    rateLimit(RateLimitName("email-verification-code")) {
        post("/v1/email-verification-codes") {
            ...
            call.respond(accountService.requestNewEmailVerificationCode(request.email, call.emailLanguage()))
        }
    }
    rateLimit(RateLimitName("password-reset-request")) {
        post("/v1/password-reset-codes") {
            ...
            call.respond(accountService.requestPasswordReset(request.email, call.emailLanguage()))
        }
    }
    ...
    authenticate("account-bearer") {
        ...
        post("/v1/email-change-current-verifications") {
            ...
            call.respond(
                accountService.verifyCurrentEmailChange(
                    accountId = principal.accountId,
                    code = request.code,
                    language = call.emailLanguage(),
                ),
        ...
        rateLimit(RateLimitName("email-change-current-verification-code")) {
            post("/v1/email-change-current-verification-codes") {
                val principal = call.principal<AccountSessionPrincipal>()
                    ?: return@post call.respond(HttpStatusCode.Unauthorized)
                call.respond(accountService.requestNewEmailChangeCurrentCode(principal.accountId, call.emailLanguage()))
            }
        }
        ...
        rateLimit(RateLimitName("email-change-new-verification-code")) {
            post("/v1/email-change-new-verification-codes") {
                val principal = call.principal<AccountSessionPrincipal>()
                    ?: return@post call.respond(HttpStatusCode.Unauthorized)
                call.respond(accountService.requestNewEmailChangeNewEmailCode(principal.accountId, call.emailLanguage()))
            }
        }
...
/**
 * The app's current UI language, taken from the request's `Accept-Language` header so account emails
 * can be localised to match what the user sees in the app. Defaults to English when the header is
 * absent or names a language the backend does not support.
 */
private fun ApplicationCall.emailLanguage(): EmailLanguage =
    EmailLanguage.fromAcceptLanguageHeader(request.headers[HttpHeaders.AcceptLanguage])
