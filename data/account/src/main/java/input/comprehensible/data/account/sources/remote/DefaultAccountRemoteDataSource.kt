...
class DefaultAccountRemoteDataSource(
    ...
    override suspend fun signIn(email: String, password: String): SignInData {
        ...
        return SignInData(token = body.accessToken, userId = body.userId)
    }
    ...
    override suspend fun requestPasswordResetCode(email: String) {
        ...
        if (!response.status.isSuccess()) {
            error("Request password reset code failed: ${response.status}")
        }
    }
...
@Serializable
private data class PasswordResetCodeRequest(val email: String)

@Serializable
private data class PasswordResetRequest(val email: String, val password: String, val code: String)
