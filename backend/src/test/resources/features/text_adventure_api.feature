Feature: Text adventure API
  ...
  Scenario: Providing the AI with inspiration words for variety
    Given the next adventure will draw inspiration from the words "ember, lantern, whisper"
    And the next adventure continuation will draw inspiration from the words "compass, thicket, river"
    ...
    When I start a text adventure in "English" with translations in "Spanish"
    And I continue the started adventure with user message "Go north"
    Then the opening scene incorporates the inspiration words "ember, lantern, whisper"
    And the adventure continuation incorporates the inspiration words "compass, thicket, river"
