...
import input.comprehensible.backend.email.AccountEmails
import input.comprehensible.backend.email.CloudflareEmailDataSource
import input.comprehensible.backend.email.EmailContent
import input.comprehensible.backend.email.EmailDataSource
import input.comprehensible.backend.email.EmailLanguage
...
class AccountService(
    ...
    fun createAccount(email: String, password: String, language: EmailLanguage = EmailLanguage.ENGLISH): AccountResult {
        ...
        val accountId = when (insertAccountResult) {
            is InsertAccountResult.AlreadyExists -> {
                if (insertAccountResult.emailVerified) {
                    sendEmail(normalizedEmail, AccountEmails.accountAlreadyExists(language))
        ...
        sendEmail(normalizedEmail, AccountEmails.verifyEmailAddress(language, verificationCode))
        return AccountResult(HttpStatusCode.OK)
    }
    ...
    fun updateMe(
        ...
        if (updateResult == EmailChangeRequestResult.AlreadyInUse) {
            sendEmail(normalizedEmail, AccountEmails.emailUpdateAlreadyInUse(language))
        } else {
            sendEmail(account[AccountsTable.email], AccountEmails.confirmEmailChange(language, currentEmailCode))
        }
    ...
    fun verifyCurrentEmailChange(
        ...
        sendEmail(verificationResult.email, AccountEmails.verifyEmailChange(language, verificationResult.code))
    ...
    fun requestPasswordReset(email: String, language: EmailLanguage = EmailLanguage.ENGLISH): HttpStatusCode {
        val normalizedEmail = normalizeEmail(email)
        if (!isValidEmail(normalizedEmail)) return HttpStatusCode.Accepted
        val account = accountsDao.findAccountByEmail(normalizedEmail)
        if (account == null) {
            sendEmail(normalizedEmail, AccountEmails.passwordResetNoAccount(language))
        ...
        sendEmail(normalizedEmail, AccountEmails.passwordResetCode(language, code))
        return HttpStatusCode.Accepted
    }

    fun requestNewEmailVerificationCode(email: String, language: EmailLanguage = EmailLanguage.ENGLISH): HttpStatusCode {
        val normalizedEmail = normalizeEmail(email)
        if (!isValidEmail(normalizedEmail)) return HttpStatusCode.Accepted
        ...
        sendEmail(normalizedEmail, AccountEmails.verifyEmailAddress(language, code))
        return HttpStatusCode.Accepted
    }

    fun requestNewEmailChangeCurrentCode(accountId: String, language: EmailLanguage = EmailLanguage.ENGLISH): HttpStatusCode {
        ...
        sendEmail(account[AccountsTable.email], AccountEmails.confirmEmailChange(language, code))
        return HttpStatusCode.Accepted
    }

    fun requestNewEmailChangeNewEmailCode(accountId: String, language: EmailLanguage = EmailLanguage.ENGLISH): HttpStatusCode {
        val code = verificationCodeProvider()
        ...
        sendEmail(pendingEmail, AccountEmails.verifyEmailChange(language, code))
        return HttpStatusCode.Accepted
    }
    ...
    private fun sendEmail(to: String, content: EmailContent) = runBlocking {
        emailDataSource.sendEmail(to = to, subject = content.subject, textBody = content.body)
    }
