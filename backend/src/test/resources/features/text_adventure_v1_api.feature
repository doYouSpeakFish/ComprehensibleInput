Feature: Text adventure v1 API
  ...
  @v1
  Scenario: The private plan and notes for an adventure are never exposed through the API
  ...
  @v1
  Scenario: The adventure plan is provided as context when generating a later AI message
    Given the AI will write a plan that includes "PLAN_CONTEXT"
    And I have an existing adventure with stored user message id "msg_user_1" and text "Abro la puerta roja"
    And the next message id is "msg_ai_1"
    When I post a message with type "AI" and parent id "msg_user_1"
    Then the response status is 201
    And the last narrator continuation prompt includes "PLAN_CONTEXT"

  @v1
  Scenario: Private narrator notes are provided as context when generating a later AI message
    Given I have an adventure whose opening narrator message has note "NOTE_CONTEXT"
    And I have stored user message id "msg_user_1" with parent id "msg_ai_root" and text "Abro la puerta roja"
    And the next message id is "msg_ai_1"
    When I post a message with type "AI" and parent id "msg_user_1"
    Then the response status is 201
    And the last narrator continuation prompt includes "NOTE_CONTEXT"
