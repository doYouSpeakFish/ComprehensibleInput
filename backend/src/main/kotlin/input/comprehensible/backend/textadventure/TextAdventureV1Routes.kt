...
fun Route.textAdventureV1Routes(textAdventureService: TextAdventureGenerationService) {
    authenticate("account-bearer") {
        post("/v1/adventures") {
            ...
            if (request.learningLanguage.isBlank() ||
                request.translationLanguage.isBlank() ||
                request.languageLevel !in SUPPORTED_LANGUAGE_LEVELS
            ) {
        ...
        adventureDeletionV1Routes(textAdventureService)
        preplannedAdventureV1Routes(textAdventureService)
    }
}

/**
 * Preplanned adventures: a catalogue of ready-made adventures a player can start. Listing returns the
 * catalogue as metadata only (never the private plan or the opening message). Starting one copies it
 * into a new private adventure owned by the player, returned just like a freshly generated adventure;
 * from then on it is played through the ordinary adventure and message endpoints, so two players who
 * start the same preplanned adventure get separate adventures and never see each other's messages.
 */
private fun Route.preplannedAdventureV1Routes(textAdventureService: TextAdventureGenerationService) {
    get("/v1/preplanned-adventures") {
        call.principal<AccountSessionPrincipal>() ?: return@get call.respond(HttpStatusCode.Unauthorized)
        call.respond(
            textAdventureService.listPreplannedAdventures(
                learningLanguage = call.request.queryParameters["learningLanguage"]?.takeIf { it.isNotBlank() },
                translationLanguage = call.request.queryParameters["translationLanguage"]?.takeIf { it.isNotBlank() },
            )
        )
    }

    post("/v1/preplanned-adventures/{preplannedAdventureId}/adventures") {
        val principal = call.principal<AccountSessionPrincipal>() ?: return@post call.respond(HttpStatusCode.Unauthorized)
        val preplannedAdventureId = requireNotNull(call.parameters["preplannedAdventureId"])
        val response = textAdventureService.startPreplannedAdventureForAccount(
            accountId = principal.accountId,
            preplannedAdventureId = preplannedAdventureId,
        )
        if (response == null) call.respond(HttpStatusCode.NotFound) else call.respond(HttpStatusCode.Created, response)
    }
}

/**
 * Deleting adventures and undoing those deletions. A deletion is modelled as a sub-resource of the
 * adventure: DELETE on the adventure creates it, and DELETE on the deletion removes it again,
 * restoring the adventure (returned in the response so clients can re-cache it).
 */
private fun Route.adventureDeletionV1Routes(textAdventureService: TextAdventureGenerationService) {
    ...
    delete("/v1/adventures/{adventureId}/deletion") {
        val principal = call.principal<AccountSessionPrincipal>() ?: return@delete call.respond(HttpStatusCode.Unauthorized)
        val adventureId = requireNotNull(call.parameters["adventureId"])
        val response = textAdventureService.restoreAdventureForAccount(
            accountId = principal.accountId,
            adventureId = adventureId,
        )
        if (response == null) call.respond(HttpStatusCode.NotFound) else call.respond(response)
