...
Use these tasks for custom `.coverage` snapshots (generated by the custom Kover coverage plugin):

```bash
# Verify snapshots match current coverage output
./gradlew koverCoverageSnapshotCheck

# Regenerate stored snapshots when coverage output changes
./gradlew koverCoverageSnapshot
```
